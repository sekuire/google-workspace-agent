...
export class GoogleWorkspaceHandler {
  ...
  private registerCapabilities(): void {
    ...
    this.capabilities.set("google:drive:search", {
      ...
      handler: async (input) => {
        return this.tools.searchDrive({
          ...
          file_type: input.file_type as "document" | "spreadsheet" | "presentation" | "any" | undefined,
          limit: input.limit as number | undefined,
        });
      },
    });
    ...
    this.capabilities.set("task:chat", {
      ...
      handler: async (input) => {
        ...
        const message = (input.message as string) || (input.description as string) || "";
  ...
  private async handleNaturalLanguageWithoutLLM(
    ...
    const message = ((input.message as string) || (input.description as string) || "").toLowerCase();
    ...
    if (message.includes("search")) {
      const queryMatch = message.match(/(?:search|find)\s+(?:for\s+)?["']?([^"']+)["']?/i);
    ...
    return {
      success: false,
      error: "Could not understand the request. Please use specific tool commands or enable LLM for natural language processing.",
    };
  }
  ...
  async handleTask(request: A2ATaskRequest): Promise<A2ATaskResponse> {
    ...
    try {
        ...
        if (taskType.startsWith("google:") || taskType.startsWith("task:")) {
          ...
          if (chatCapability) {
            const result = await this.executeWithTimeout(
              chatCapability.handler({ ...input, description: request.description }, this.agent),
              timeoutMs
            );
      ...
      console.log(`[A2A] Task ${taskId} completed in ${Date.now() - startTime}ms`);
