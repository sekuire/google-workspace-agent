...
export class GoogleDocsTools {
	...
	async appendToDocument(params: {
		...
		try {
			const doc = await this.client.appendToDocument(
				params.document_id,
				params.content,
			);
		...
		switch (toolName) {
			case "create_document":
				return this.createDocument(
					params as { title: string; content?: string; folder_id?: string },
				);
			case "read_document":
				return this.readDocument(params as { document_id: string });
			case "update_document":
				return this.updateDocument(
					params as { document_id: string; content: string },
				);
			case "append_to_document":
				return this.appendToDocument(
					params as { document_id: string; content: string },
				);
			case "list_documents":
				return this.listDocuments(
					params as { folder_id?: string; query?: string; limit?: number },
				);
			case "search_drive":
				return this.searchDrive(
					params as {
						query: string;
						file_type?: "document" | "spreadsheet" | "presentation" | "any";
						limit?: number;
